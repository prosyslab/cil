...
type paramkind = PKNone | PKThis | PKOffset of attrparam
...
let loc_comp l1 l2 =
  if String.compare l1.A.filename l2.A.filename != 0 then None
  else if l1.A.lineno > l2.A.lineno then Some 1
  else if l2.A.lineno > l1.A.lineno then Some (-1)
  else if l1.A.byteno > l2.A.byteno then Some 1
  else if l2.A.byteno > l1.A.byteno then Some (-1)
  else Some 0
...
let get_comments l =
  let cabsl =
    { A.lineno = l.line; A.filename = l.file; A.byteno = l.byte; A.ident = 0 }
  in
  ...
  let rec loop i cl =
    if i < 0 then cl
      ...
      if String.compare cabsl.A.filename l'.A.filename != 0 then loop (i - 1) cl
      else if b then cl
      else
        let _ = GA.set CH.commentsGA i (l', c, true) in
        loop (i - 1) (c :: cl)
  in
  List.rev (loop s [])
...
let get_loop_condition b =
  ...
  let rec get_cond_from_if if_stm =
    ...
    | If (e, tb, fb, _) -> (
        ...
        | [], { skind = Break _; _ } :: _ -> Some (UnOp (LNot, e, intType))
        | ({ skind = If (_, _, _, _); _ } as s) :: _, [] -> (
            let teo = get_cond_from_if s in
            match teo with
            | None -> None
            | Some te -> Some (BinOp (LAnd, e, EC.stripNopCasts te, intType)))
        | [], ({ skind = If (_, _, _, _); _ } as s) :: _ -> (
        ...
        | ( { skind = Break _; _ } :: _,
            ...
            match feo with
            | None -> None
            | Some fe -> Some (BinOp (LOr, e, EC.stripNopCasts fe, intType)))
        | ( ({ skind = If (_, _, _, _); _ } as s) :: _,
            { skind = Break _; _ } :: _ ) -> (
            let teo = get_cond_from_if s in
            match teo with
            ...
            | Some te ->
                Some
                  (BinOp
                     (LOr, UnOp (LNot, e, intType), EC.stripNopCasts te, intType))
            )
        | ( ({ skind = If (_, _, _, _); _ } as ts) :: _,
            ({ skind = If (_, _, _, _); _ } as fs) :: _ ) -> (
        ...
        | _, _ ->
            if !debug then
              ignore
                (E.log "cond_finder: branches of %a not good\n" d_stmt if_stm);
            None)
    | _ ->
        if !debug then
          ignore (E.log "cond_finder: %a not an if\n" d_stmt if_stm);
        None
  in
  ...
  | s :: _ ->
      if !debug then
        ignore (E.log "checkMover: %a is first, not an if\n" d_stmt s);
      (None, sl)
  | [] ->
      if !debug then ignore (E.log "checkMover: no statements in loop block?\n");
      (None, sl)

class zraCilPrinterClass : cilPrinter =
  object (self)
    ...
    method private getEnvVi (v : varinfo) : varinfo =
      try
        if H.mem lenvHtbl v.vname then H.find lenvHtbl v.vname
    ...
    method private checkViAndWarn (v : varinfo) =
      if not (self#checkVi v) then
        ignore
          (warn
             "mentioned variable %s and its entry in the current environment \
              have different varinfo."
             v.vname)
      ...
      if IH.mem RCT.iioh v.vid then
        ...
        | Some (Call (_, e, el, l)) ->
            ...
            let c =
              ...
              | TFun (rt, _, _, _)
                when not (Util.equals (typeSig rt) (typeSig v.vtype)) ->
        ...
        | _ ->
            if IH.mem RCT.incdecHash v.vid then
              ...
              match b with
              | PlusA | PlusPI | IndexPI -> text rhsvi.vname ++ text "++"
              | MinusA | MinusPI -> text rhsvi.vname ++ text "--"
              | _ ->
                  E.s
                    (E.error
                       "zraCilPrinterClass.pVar: unexpected op for inc/dec")
            else (
              self#checkViAndWarn v;
              text v.vname)
        ...
        | MinusA | MinusPI -> text rhsvi.vname ++ text "--"
        | _ ->
            E.s (E.error "zraCilPrinterClass.pVar: unexpected op for inc/dec")
      else (
        self#checkViAndWarn v;
        text v.vname)
    ...
    method! pVDecl () (v : varinfo) =
      ...
      let _ =
        if H.mem lenvHtbl v.vname && not (self#checkVi v) then
          ignore
            (warn
               "name %s has already been declared locally with different \
                varinfo"
               v.vname)
        else if H.mem genvHtbl v.vname && not (self#checkVi v) then
          ignore
            (warn
               "name %s has already been declared globally with different \
                varinfo"
               v.vname)
        else if not v.vglob then (
          if !debug then
            ignore (E.log "zrapp: adding %s to local pp environment\n" v.vname);
          H.add lenvHtbl v.vname v)
        else (
          if !debug then
            ignore (E.log "zrapp: adding %s to global pp envirnoment\n" v.vname);
          H.add genvHtbl v.vname v)
      in
    ...
    method! pAttr (Attr (an, args) : attribute) : doc * bool =
      if not !deputyAttrs then super#pAttr (Attr (an, args))
      else
        match (an, args) with
        ...
        | "bounds", [ a1; a2 ] -> (
            ...
            | PKThis, PKThis -> (text "COUNT(0)", false)
            | PKThis, PKOffset (AInt 1) -> (text "SAFE", false)
            | PKThis, PKOffset a ->
                (nil, false)
                (*if !showBounds then
        ...
        | "size", [ a ] -> (dprintf "SIZE(%a)" self#pAttrParam a, false)
        | "fancywhen", [ AInt i ] ->
            (nil, false)
            (*dprintf "WHEN(%a)" self#pExp (getBoundsExp i), false*)
        | "when", [ a ] -> (dprintf "WHEN(%a)" self#pAttrParam a, false)
        | "nullterm", [] -> (text "NT", false)
        | "assumeconst", [] -> (text "ASSUMECONST", false)
        | "trusted", [] -> (text "TRUSTED", false)
        | "poly", [ a ] -> (dprintf "POLY(%a)" self#pAttrParam a, false)
        | "poly", [] -> (text "POLY", false)
        | "sentinel", [] -> (text "SNT", false)
        | "nonnull", [] -> (text "NONNULL", false)
        | "_ptrnode", [ AInt n ] ->
            (nil, false)
            (*if !Doptions.emitGraphDetailLevel >= 3 then
                ...
                nil, false*)
        | "missing_annot", _ ->
            (* Don't bother printing thess *)
            (nil, false)
        | _ -> super#pAttr (Attr (an, args))
    ...
    method! pGlobal () (g : global) : doc =
      ...
      | GEnumTagDecl (enum, l) ->
          ...
          self#pLineDirective l ++ text ("enum " ^ enum.ename ^ ";\n")
      | GCompTag (comp, l) ->
      ...
      | GCompTagDecl (comp, l) ->
          (* This is a declaration of a tag *)
          self#pLineDirective l ++ text (compFullName comp) ++ text ";\n"
      | GVar (vi, io, l) ->
          self#pLineDirective ~forcefile:true l
          ++ self#pVDecl () vi ++ chr ' '
          ++ (match io.init with
      ...
      | GVarDecl (vi, l) ->
          ...
          if (not !printCilAsIs) && H.mem builtins vi.vname then
    ...
    method private pFunDecl () f =
      H.add genvHtbl f.svar.vname f.svar;
      (* add function to global env *)
      H.clear lenvHtbl;
      (* new local environment *)
      (* add the arguments to the local environment *)
      List.iter (fun vi -> H.add lenvHtbl vi.vname vi) f.sformals;
      let nf = if !doElimTemps then RCT.eliminate_temps f else f in
      let decls =
        docList ~sep:line
          (fun vi -> self#pVDecl () vi ++ text ";")
          () nf.slocals
      in
      self#pVDecl () nf.svar ++ line ++ text "{ "
      ++ (align
        (* locals. *)
        ++ decls
         ++ line ++ line
...
type outfile = { fname : string; fchan : out_channel }
...
let openFile (what : string) (takeit : outfile -> unit) (fl : string) =
  if !E.verboseFlag then ignore (Printf.printf "Setting %s to %s\n" what fl);
  try takeit { fname = fl; fchan = open_out fl }
  with _ -> raise (Arg.Bad ("Cannot open " ^ what ^ " file " ^ fl))
...
let feature =
  {
    ...
    fd_enabled = false;
    fd_description =
      "pretty printing with checks for name conflicts and\n\

...
let ih_inter ih1 ih2 =
  ...
  IH.iter
    (fun id vi -> if not (IH.mem ih2 id) then IH.remove ih' id else ())
    ih1;
  ih'
...
let ih_union ih1 ih2 =
  let ih' = IH.copy ih1 in
  IH.iter
    (fun id vi -> if not (IH.mem ih' id) then IH.add ih' id vi else ())
    ih2;
  ih'
...
let iosh_singleton_lookup iosh vi =
  if IH.mem iosh vi.vid then
    ...
    if not (IOS.cardinal ios = 1) then None else IOS.choose ios
  else None

(* IOS.t IH.t -> varinfo -> IOS.t *)
let iosh_lookup iosh vi =
  if IH.mem iosh vi.vid then Some (IH.find iosh vi.vid) else None
...
let iosh_defId_find iosh defId =
  ...
  let get_vid vid ios io =
    ...
    | None ->
        let there =
          IOS.exists (function None -> false | Some i' -> defId = i') ios
        in
...
let iosh_equals iosh1 iosh2 =
  ...
  if not (IH.length iosh1 = IH.length iosh2) then (
    if !debug then
      ignore
        (E.log "iosh_equals: length not same: %d %d\n" (IH.length iosh1)
           (IH.length iosh2));
    false)
  else
    IH.fold
      (fun vid ios b ->
        if not b then b
          ...
          try
            let ios2 = IH.find iosh2 vid in
            if not (IOS.compare ios ios2 = 0) then (
              if !debug then
                ignore (E.log "iosh_equals: sets for vid %d not equal\n" vid);
              false)
            else true
          with Not_found ->
            if !debug then
              ignore (E.log "iosh_equals: vid %d not in iosh2\n" vid);
            false)
      iosh1 true
...
let proc_defs vs iosh f =
  let pd vi =
    let newi = f () in
    if !debug then ignore (E.log "proc_defs: genning %d\n" newi);
...
let instrRDs il sid (ivih, s, iosh) out =
  if Hashtbl.mem iRDsHtbl (sid, out) then Hashtbl.find iRDsHtbl (sid, out)
  else
    ...
    let proc_one hil i =
      ...
      | [] ->
          ...
          if UD.VS.is_empty defd then
            [ (*if !debug then print_instr i ((), s, iosh);*) ((), s, iosh) ]
          else
...
let getDefRhs didstmh stmdat defId =
  if IH.mem rhsHtbl defId then IH.find rhsHtbl defId
  else
    let stm =
      try IH.find didstmh defId
      with Not_found -> E.s (E.error "getDefRhs: defId %d not found" defId)
    in
    let _, s, iosh =
      try IH.find stmdat stm.sid
      with Not_found -> E.s (E.error "getDefRhs: sid %d not found" stm.sid)
    in
    ...
    | Instr il -> (
        ...
        try
          ...
          try
            let (i, (_, _, diosh)), (_, _, iosh_in) =
              List.find
                (fun ((i, (_, _, iosh')), _) ->
                  match
                    time "iosh_defId_find" (iosh_defId_find iosh') defId
                  with
                  | Some vid -> (
                      match i with
                      | Set ((Var vi', NoOffset), _, _) ->
                          vi'.vid = vid (* _ -> NoOffset *)
                      ...
                      | Asm (_, _, sll, _, _, _) ->
                          List.exists
                            (function
                              | _, _, (Var vi', NoOffset) -> vi'.vid = vid
                              | _ -> false)
                            sll
                      | _ -> false)
                  | None -> false)
            ...
            | Set ((lh, _), e, _) -> (
                ...
                | _ ->
                    E.s (E.error "Reaching Defs getDefRhs: right vi not first"))
            | Call (lvo, e, el, _) ->
          ...
          with Not_found ->
            if !debug then
              ignore (E.log "getDefRhs: No instruction defines %d\n" defId);
            IH.add rhsHtbl defId None;
            None
        with Invalid_argument _ -> None)
    | _ ->
        E.s
          (E.error "getDefRhs: defining statement not an instruction list %d"
             defId)
(*None*)
      ...
      ++ IOS.fold
           (fun io d ->
             ...
             | Some i -> (
                 ...
                 | None -> d ++ num i
                 | Some (RDExp e, _, _) -> d ++ num i ++ text " " ++ d_exp () e
                 | Some (RDCall c, _, _) ->
...
module ReachingDef = struct
  ...
  let num_defs stm =
    ...
    | Instr il ->
        List.fold_left
          ...
          0 il
    | _ ->
        let _, d = UD.computeUseDefStmtKind stm.skind in
        UD.VS.cardinal d
  ...
  let combinePredecessors (stm : stmt) ~(old : t) ((_, s, iosh) : t) =
    match old with
    | _, os, oiosh ->
        if time "iosh_equals" (iosh_equals oiosh) iosh then None
        else Some ((), os, time "iosh_combine" (iosh_combine oiosh) iosh)
  ...
  let doStmt stm (_, s, iosh) =
    if not (IH.mem sidStmtHash stm.sid) then IH.add sidStmtHash stm.sid stm;
    if !debug then ignore (E.log "RD: looking at %a\n" d_stmt stm);
    match L.getLiveSet stm.sid with
    | None -> DF.SDefault
    | Some vs -> DF.SUse ((), s, iosh_filter_dead iosh vs)
  (*DF.SDefault*)
...
let iosh_none_fill iosh vil =
  List.iter (fun vi -> IH.add iosh vi.vid (IOS.singleton None)) vil
...
let computeRDs fdec =
  try
    ...
    if !debug then ignore (E.log "computeRDs: fst_stm.sid=%d\n" fst_stm.sid);
    RD.compute [ fst_stm ];
    if compare fdec.svar.vname !debug_fn = 0 then debug := false
...
let getRDs sid =
  try Some (IH.find ReachingDef.stmtStartData sid) with Not_found -> None
(*    E.s (E.error "getRDs: sid %d not found" sid) *)

let getDefIdStmt defid =
  try Some (IH.find ReachingDef.defIdStmtHash defid) with Not_found -> None

let getStmt sid =
  try Some (IH.find ReachingDef.sidStmtHash sid) with Not_found -> None

(* returns the rhs for the definition *)
let getSimpRhs defId =
  let rhso =
    getDefRhs ReachingDef.defIdStmtHash ReachingDef.stmtStartData defId
  in
  match rhso with None -> None | Some (r, _, _) -> Some r
...
class rdVisitorClass =
  object (self)
    ...
    method get_cur_iosh () =
      ...
      | None -> (
          match getRDs sid with None -> None | Some (_, _, iosh) -> Some iosh)

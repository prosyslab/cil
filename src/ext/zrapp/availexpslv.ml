...
module LvExpHash = H.Make (struct
  type t = lval
...
let lvh_equals lvh1 lvh2 =
  if not (LvExpHash.length lvh1 = LvExpHash.length lvh2) then false
  else
    LvExpHash.fold
      (fun lv e b ->
        if not b then b
          ...
          try
            let e2 = LvExpHash.find lvh2 lv in
            if not (compareExpStripCasts e e2) then false else true
          with Not_found -> false)
      lvh1 true

let lvh_pretty () lvh =
  LvExpHash.fold
    (fun lv e d -> d ++ line ++ d_lval () lv ++ text " -> " ++ d_exp () e)
    lvh nil
...
let lvh_combine lvh1 lvh2 =
  if !debug then
    ignore
      (E.log "lvh_combine: combining %a\n and\n %a\n" lvh_pretty lvh1 lvh_pretty
         lvh2);
  ...
  LvExpHash.iter
    (fun lv e1 ->
      try
        let e2l = LvExpHash.find_all lvh2 lv in
        if
          not (List.exists (fun e2 -> compareExpStripCasts e1 e2) e2l)
          (* remove things from eh' that eh2 doesn't have *)
        then (
          let e1l = LvExpHash.find_all lvh' lv in
          let e1l' =
            List.filter (fun e -> not (compareExpStripCasts e e1)) e1l
          in
          LvExpHash.remove lvh' lv;
          List.iter (fun e -> LvExpHash.add lvh' lv e) e1l')
      with Not_found -> LvExpHash.remove lvh' lv)
    lvh1;
...
class memReadOrAddrOfFinderClass br =
  object (self)
    inherit nopCilVisitor

    method! vexpr e =
      match e with
      | AddrOf (Mem _, _) | StartOf (Mem _, _) | Lval (Mem _, _) ->
          br := true;
          SkipChildren
...
let lvh_kill_mem lvh =
  LvExpHash.iter
    (fun lv e ->
      ...
      | _ ->
          if exp_has_mem_read e || lval_has_mem_read lv then
            LvExpHash.remove lvh lv)
    lvh

(* need to kill exps containing a particular vi sometimes *)
class viFinderClass vi br =
  object (self)
    inherit nopCilVisitor

    method! vvrbl vi' =
      if vi.vid = vi'.vid then (
        br := true;
        SkipChildren)
      else DoChildren
  end
...
let lvh_kill_vi lvh vi =
  LvExpHash.iter
    (fun lv e ->
      if exp_has_vi vi e || lval_has_vi vi lv then LvExpHash.remove lvh lv)
    lvh

(* need to kill exps containing a particular lval sometimes *)
class lvalFinderClass lv br =
  object (self)
    inherit nopCilVisitor

    method! vlval l =
      if compareLval l lv then (
        br := true;
        SkipChildren)
      else DoChildren
  end
...
let lval_has_lval lv (host, hostoff) =
  let br = ref false in
  let vis = new lvalFinderClass lv br in
  (match host with Mem e -> ignore (visitCilExpr vis e) | _ -> ());
...
let lvh_kill_lval lvh lv =
  LvExpHash.iter
    (fun lv' e ->
      if exp_has_lval lv e || lval_has_lval lv lv' then LvExpHash.remove lvh lv')
    lvh

class volatileFinderClass br =
  object (self)
    inherit nopCilVisitor

    method! vexpr e =
      if hasAttribute "volatile" (typeAttrs (typeOf e)) then (
        br := true;
        SkipChildren)
      else DoChildren
  end
...
class addrOfOrGlobalFinderClass br =
  object (self)
    inherit nopCilVisitor
...
let lvh_kill_addrof_or_global lvh =
  LvExpHash.iter
    (fun lv e -> if lval_has_addrof_or_global lv then LvExpHash.remove lvh lv)
    lvh

let lvh_handle_inst i lvh =
  if !ignore_inst i then lvh
  else
    match i with
    | Set (lv, e, _) -> (
        match lv with
        ...
        | _ ->
            ...
            if !debug then
              ignore
                (E.log "lvh_handle_inst: %a is volatile. killing %a\n" d_exp e
                   d_lval lv);
            LvExpHash.remove lvh lv;
            lvh_kill_lval lvh lv;
            lvh)
    ...
    | Asm (_, _, _, _, _, _) ->
        ...
        UD.VS.iter (fun vi -> lvh_kill_vi lvh vi) d;
        lvh

module AvailableExps = struct
  ...
  let combinePredecessors (stm : stmt) ~(old : t) (lvh : t) =
    if time "lvh_equals" (lvh_equals old) lvh then None
    else Some (time "lvh_combine" (lvh_combine old) lvh)
...
let computeAEs fd =
  ...
  with
  | Failure _ -> if !debug then ignore (E.log "fn w/ no stmts?\n")
  | Not_found -> if !debug then ignore (E.log "no data for first_stm?\n")

(* get the AE data for a statement *)
let getAEs sid =
  try Some (IH.find AvailableExps.stmtStartData sid) with Not_found -> None
...
class aeVisitorClass =
  object (self)
    ...
    method! vinst i =
      if !debug then
        ignore
          (E.log "aeVist: before %a, ae_dat_lst is %d long\n" d_instr i
             (List.length ae_dat_lst));
      try
    ...
    method get_cur_eh () =
      match cur_ae_dat with None -> getAEs sid | Some eh -> Some eh
  end

...
let unbox_int_type (ye : typ) : int * sign =
  ...
  let s =
    match tp with
    | TInt (i, _) -> if isSigned i then Signed else Unsigned
...
let ocaml_int_to_cil v n s =
  ...
  let i =
    match s with
    | Signed ->
        if n = char_size then ISChar
        else if n = int_size then IInt
        else if n = short_size then IShort
        else if n = long_size then ILong
        else if n = longlong_size then ILongLong
        else raise Weird_bitwidth
    | Unsigned ->
        if n = char_size then IUChar
        else if n = int_size then IUInt
        else if n = short_size then IUShort
        else if n = long_size then IULong
        else if n = longlong_size then IULongLong
        else raise Weird_bitwidth
  in
  kinteger64 i v
...
class attribute_checker target rflag =
  object (self)
    ...
    method! vtype t =
      match t with
      | TComp (cinfo, a) ->
          if not (List.exists (fun x -> cinfo.cname = x) !visited) then (
            visited := cinfo.cname :: !visited;
            List.iter
              (fun f ->
                if hasAttribute target f.fattr then rflag := true
                else
                  ignore
                    (visitCilType (new attribute_checker target rflag) f.ftype))
              cinfo.cfields);
          DoChildren
      | TNamed (t1, a) ->
          if not (List.exists (fun x -> t1.tname = x) !visited) then (
...
let compare_exp x y = compare x y

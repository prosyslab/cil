...
let eh_equals eh1 eh2 =
  if not (IH.length eh1 = IH.length eh2) then false
  else
    IH.fold
      (fun vid e b ->
        if not b then b
...
let eh_pretty () eh =
  ...
  ++ seq ~sep:line
       ~doit:(fun (vid, e) ->
         text "AE:vid:" ++ num vid ++ text ": " ++ d_exp () e)
       ~elements:(IH.tolist eh)
...
let eh_kill_mem eh =
  IH.iter (fun vid e -> if exp_has_mem_read e then IH.remove eh vid) eh
...
let eh_kill_vi eh vi =
  IH.iter (fun vid e -> if exp_has_vi vi e then IH.remove eh vid) eh
...
let eh_kill_lval eh lv =
  IH.iter (fun vid e -> if exp_has_lval lv e then IH.remove eh vid) eh
...
let eh_kill_addrof_or_global eh =
  ...
  IH.iter
    (fun vid e ->
      try
        let vi = IH.find varHash vid in
        if vi.vaddrof then (
          if !debug then
            ignore (E.log "eh_kill: %s has its address taken\n" vi.vname);
          IH.remove eh vid)
...
let eh_handle_inst i eh =
  if !ignore_inst i then eh
    ...
    | Asm (_, _, _, _, _, _) ->
        ...
        UD.VS.iter (fun vi -> eh_kill_vi eh vi) d;
        eh
...
module AvailableExps = struct
  ...
  let combinePredecessors (stm : stmt) ~(old : t) (eh : t) =
    if time "eh_equals" (eh_equals old) eh then None
    else Some (time "eh_combine" (eh_combine old) eh)
...
class varHashMakerClass =
  object (self)
    ...
    method! vvrbl vi =
      if not (IH.mem varHash vi.vid) then (
        if !debug && vi.vglob then ignore (E.log "%s is global\n" vi.vname);
        if !debug && not vi.vglob then
          ignore (E.log "%s is not global\n" vi.vname);

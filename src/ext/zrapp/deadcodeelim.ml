...
let callHasNoSideEffects : (instr -> bool) ref = ref (fun _ -> false)
...
class usedDefsCollectorClass =
  object (self)
      ...
      UD.VS.iter
        (fun vi ->
          if IH.mem iosh vi.vid then (
            ...
            if !debug then
              ignore
                (E.log "DCE: IOS size for vname=%s at stmt=%d: %d\n" vi.vname
                   sid (RD.IOS.cardinal ios));
            RD.IOS.iter
              (function
                | Some i ->
                    if !debug then
                      ignore (E.log "DCE: def %d used: %a\n" i d_exp e);
                    usedDefsSet := IS.add i !usedDefsSet
                | None -> ())
              ios)
          else if !debug then
            ignore
              (E.log "DCE: vid %d:%s not in stm:%d iosh at %a\n" vi.vid vi.vname
                 sid d_plainexp e))
        u
    ...
    method! vexpr e =
      ...
      | Some iosh ->
          self#add_defids iosh e u;
          DoChildren
      | None ->
          if !debug then
            ignore (E.log "DCE: use but no rd data: %a\n" d_plainexp e);
          DoChildren
...
let el_has_volatile = List.fold_left (fun b e -> b || exp_has_volatile e) false
(***************************************************)
    ...
    method! vstmt stm =
        ...
        UD.VS.fold
          (fun vi s ->
            if IH.mem iosh vi.vid then
              let ios = IH.find iosh vi.vid in
              RD.IOS.fold
                (fun io s -> match io with None -> s | Some i -> IS.add i s)
                ios s
            else s)
          vis IS.empty
      in
      ...
      let check_defid i instruses iosh defid =
        ...
        try
          ...
          if IH.mem sidUseSetHash defid then (
            ...
            UD.VS.exists (fun vi -> vi.vglob) instruses
          then true
          else
            let instruses = viSetToDefIdSet iosh instruses in
            IS.fold
              (fun i' b ->
                if not (IS.mem i' instruses) then (
                  if !debug then
                    ignore (E.log "i not in instruses: %a\n" d_instr i);
                  true)
                else
                  ...
                  ||
                  if not (IS.equal i'_uses (IS.singleton defid)) then (
                    IS.iter
                      (fun iu ->
                        match RD.getSimpRhs iu with
                        | Some (RD.RDExp e) ->
                            if !debug then
                              ignore
                                (E.log "i' had other than one use: %d: %a\n"
      ...
      let test (i, (_, s, iosh)) =
        ...
        | Call (Some (Var vi, NoOffset), Lval (Var vf, NoOffset), el, l) ->
            if not (!callHasNoSideEffects i) then (
              if !debug then
                ignore (E.log "found call w/ side effects: %a\n" d_instr i);
              true)
            else (
              if !debug then
                ignore (E.log "found call w/o side effects: %a\n" d_instr i);
              vi.vglob || Ciltools.is_volatile_vi vi || el_has_volatile el
              ||
              let uses, defd = UD.computeUseDefInstr i in
              let rec loop n =
                n >= 0 && (check_defid i uses iosh (n + s) || loop (n - 1))
              in
              ...
              (incr removedCount;
               false))
        | Call _ -> true
        | Set (lh, e, _) when compareExpStripCasts (Lval lh) e ->
            false (* filter x = x *)
...
let elim_dead_code_fp (fd : fundec) : fundec =
  ...
  let rec loop fd =
    ...
    ignore
      (time "ud-collector"
         (visitCilFunction (new usedDefsCollectorClass :> cilVisitor))
         fd);
    let fd' =
      time "useless-elim" (visitCilFunction (new uselessInstrElim)) fd
    in
...
class deadCodeElimClass : cilVisitor =
  object (self)
    inherit nopCilVisitor

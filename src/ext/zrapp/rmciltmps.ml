...
let time s f a = if !doTime then S.time s f a else f a
...
let getDefRhs =
  RD.getDefRhs RD.ReachingDef.defIdStmtHash RD.ReachingDef.stmtStartData
  ...
  object (self)
    ...
    method vvrbl vi =
      if vi.vglob then (
        ...
        if !debug then
          ignore
            (E.log "memReadOrAddrOfFinder: %s has its address taken\n" vi.vname);
        exp_ok := false;
        SkipChildren)
      else (
        if !debug then
          ignore
            (E.log "memReadOrAddrOfFinder: %s does not have its address taken\n"
               vi.vname);
        DoChildren)
  end
...
let exp_is_ok_replacement e =
  if !debug then
    ignore
      (E.log "exp_is_ok_replacement: in exp_is_ok_replacement with %a\n" d_exp e);
  exp_ok := true;
...
let writes_between f dsid sid =
  if Hashtbl.mem wbHtbl (dsid, sid) then Hashtbl.find wbHtbl (dsid, sid)
  else
    ...
    let rec dfs goal b start =
      if !debug then
        ignore (E.log "writes_between: dfs visiting %a\n" d_stmt start);
      if start.sid = goal.sid then (
        let wh = find_write start in
        if !debug && b then
          ignore (E.log "writes_between: start=goal and found a write\n");
        if !debug && not b then
          ignore (E.log "writes_between: start=goal and no write\n");
        if !debug && wh then
          ignore (E.log "writes_between: start=goal and write here\n");
        if !debug && not wh then
          ignore (E.log "writes_between: start=goal and no write here\n");
        b || find_write start)
      else if
        (* if time "List.mem1" (List.mem start.sid) (!visited_sid_lr) then false else *)
        IS.mem start.sid !visited_sid_isr
      then false
      else
        let w = find_write start in
        if !debug && w then
          ignore (E.log "writes_between: found write %a\n" d_stmt start);
        visited_sid_isr := IS.add start.sid !visited_sid_isr;
        let rec proc_succs sl =
          match sl with
          | [] -> false
  ...
  UD.VS.fold
    (fun vi b ->
      ...
      | Some curid, Some defid ->
          if !debug then
            ignore
              (E.log "verify_unmodified: curido: %d defido: %d\n" curid defid);
          curid = defid && b
      | None, None ->
          if not (UD.VS.mem vi fdefs) then (
            if !debug then
              ignore
                (E.log "verify_unmodified: %s not defined in function\n"
                   vi.vname);
            b)
          else
            (* if the same set of definitions reaches, we can replace, also *)
            let curios =
              try IH.find curiosh vi.vid with Not_found -> RD.IOS.empty
            in
            let defios =
              try IH.find defiosh vi.vid with Not_found -> RD.IOS.empty
            in
      ...
      | _, _ ->
          if !debug then
            ignore
              (E.log
                 "verify_unmodified: %s has conflicting definitions. cur: %a\n\
                 \ def: %a\n"
...
let ok_to_replace vi curiosh sid defiosh dsid f r =
  let uses, safe =
    ...
    | RD.RDCall (Call (_, _, el, _) as i) ->
        let safe =
          List.fold_left (fun b e -> exp_is_ok_replacement e && b) true el
        in
  ...
  let target_addrof =
    if vi.vaddrof || vi.vglob then (
      if !debug then
        ignore
          (E.log
             "ok_to_replace: target %s had its address taken or is a global\n"
             vi.vname);
      true)
    else (
      if !debug then
        ignore
          (E.log "ok_to_replace: target %s does not have its address taken\n"
             vi.vname);
      false)
  in
  let writes =
    if safe && not target_addrof then false
    else time "writes_between" (writes_between f dsid) sid
  in
  if ((not safe) || target_addrof) && writes then (
    if !debug then
      ignore
        (E.log
           "ok_to_replace: replacement not safe because of pointers or addrOf\n");
    false)
  else
    let fdefs = collect_fun_defs f in
    let _ =
      if !debug then
        ignore (E.log "ok_to_replace: card fdefs = %d\n" (UD.VS.cardinal fdefs))
    in
    let _ =
      if !debug then
        ignore (E.log "ok_to_replace: card uses = %d\n" (UD.VS.cardinal uses))
    in
...
class useListerClass (defid : int) (vi : varinfo) =
  object (self)
    ...
    method vexpr e =
      ...
      | Lval (Var vi', off) -> (
          ...
          | Some iosh ->
              ...
              if Util.equals vi vi' && exists then (
                useList := sid :: !useList;
                DoChildren)
              else DoChildren
...
let ok_to_replace_with_incdec curiosh defiosh f id vi r =
  ...
  | RD.RDExp (Lval (Var rhsvi, NoOffset)) -> (
      ...
      | Some curid, _ -> (
          let defios =
            try IH.find defiosh rhsvi.vid with Not_found -> RD.IOS.empty
          in
          ...
          | None ->
              if !debug then
                ignore
                  (E.log "ok_to_replace: couldn't get rhs for redef: %d\n" curid);
              None
          | Some (redefrhs, _, redefiosh) -> (
              ...
              | None ->
                  if !debug then
                    ignore
                      (E.log
                         "ok_to_replace: conflicting defs of %s reach redef of \
                          %s\n"
              ...
              | Some tmprdid -> (
                  if not (tmprdid = id) then (
                    if !debug then
                      ignore
                        (E.log
                           "ok_to_replace: initial def of %s doesn't reach \
                            redef of %s\n"
                           vi.vname rhsvi.vname);
                    None)
                  else
                    let redefios =
                      try IH.find redefiosh rhsvi.vid
                      with Not_found -> RD.IOS.empty
                    in
                    let curdef_stmt =
                      ...
                      with Not_found ->
                        E.s
                          (E.error
                             "ok_to_replace: couldn't find statement defining \
                              %d"
                             curid)
                    in
                    if not (RD.IOS.compare defios redefios = 0) then (
                      if !debug then
                        ignore
                          (E.log
                             "ok_to_replace: different sets of definitions of \
                              %s reach the def of %s and the redef of %s\n"
                             rhsvi.vname vi.vname rhsvi.vname);
                      None)
                    else
                      match redefrhs with
                      | RD.RDExp e -> (
                          match inc_or_dec e rhsvi with
                          | Some PlusA ->
                              if num_uses () = 1 then
                                ...
                                if !debug then
                                  ignore
                                    (E.log
                                       "ok_to_replace: tmp used more than once\n");
                                None)
                          | Some MinusA ->
                              if num_uses () = 1 then
                          ...
                          | None ->
                              if !debug then
                                ignore
                                  (E.log
                                     "ok_to_replace: redef isn't adding or \
                                      subtracting one from itself\n");
                              None
                          | _ ->
                              E.s
                                (E.error
                                   "ok_to_replace: unexpected op in inc/dec \
                                    info."))
                      | _ ->
                          if !debug then
                            ignore (E.log "ok_to_replace: redef a call\n");
                          None)))
      | _ ->
          if !debug then
            ignore
              (E.log "ok_to_replace: %s has conflicting definitions\n"
                 rhsvi.vname);
          None)
  | _ ->
      if !debug then ignore (E.log "ok_to_replace: rhs not of correct form\n");
      None
...
let check_form s f =
  ...
  | Suffix sfx ->
      ...
      slen >= frmlen && compare (String.sub s (slen - frmlen) frmlen) sfx = 0
  | Prefix pfx ->
      let frmlen = String.length pfx in
      String.length s >= frmlen && compare (String.sub s 0 frmlen) pfx = 0
  | Exact ext ->
      let frmlen = String.length ext in
      String.length s = frmlen && compare s ext = 0

(* check a name against a list of forms
   if it matches any then return true *)
(* string -> nameform list -> bool *)
let check_forms s fl = List.fold_left (fun b f -> b || check_form s f) false fl

let forms =
  [
    Exact "tmp"; Prefix "tmp___"; Prefix "__cil_tmp"; Suffix "__e"; Suffix "__b";
  ]
...
let varXformClass action data sid fd nofrm =
  object (self)
    inherit nopCilVisitor
...
let lvalXformClass action data sid fd nofrm =
  object (self)
    inherit nopCilVisitor
  ...
  if IH.mem iosh vi.vid then
    ...
    let ios' =
      RD.IOS.filter
        ...
        ios
    in
    if not (RD.IOS.cardinal ios' = 1) then (
      if !debug then
        ignore
          (E.log "iosh_get_useful_def: multiple different defs of %d:%s(%d)\n"
             vi.vid vi.vname (RD.IOS.cardinal ios'));
      None)
    else RD.IOS.choose ios'
  else (
    if !debug then
      ignore (E.log "iosh_get_useful_def: no def of %s reaches here\n" vi.vname);
    None)
...
let ae_tmp_to_exp eh sid vi fd nofrm =
  if nofrm || check_forms vi.vname forms then
    try
      ...
      if !debug then
        ignore (E.log "tmp_to_exp: changing %s to %a\n" vi.vname d_plainexp e);
      match e with
      | Const (CStr _) | Const (CWStr _) -> None (* don't fwd subst str lits *)
      | _ ->
...
let rd_tmp_to_exp iosh sid vi fd nofrm =
  if nofrm || check_forms vi.vname forms then
    let ido = iosh_get_useful_def iosh vi in
    match ido with
    | None ->
        if !debug then
          ignore (E.log "tmp_to_exp: non-single def: %s\n" vi.vname);
        None
    | Some id -> (
        let defrhs = time "getDefRhs" getDefRhs id in
        match defrhs with
        | None ->
        ...
        | Some ((RD.RDExp e as r), dsid, defiosh) ->
            if
              time "ok_to_replace" (ok_to_replace vi iosh sid defiosh dsid fd) r
            then (
              if !debug then
                ignore
                  (E.log "tmp_to_exp: changing %s to %a\n" vi.vname d_plainexp e);
              match e with
              | Const (CStr _) | Const (CWStr _) -> None
              | _ ->
                  rd_tmp_to_exp_change := true;
                  Some e)
            else (
              if !debug then
                ignore (E.log "tmp_to_exp: not ok to replace %s\n" vi.vname);
              None)
        | _ ->
            if !debug then
              ignore (E.log "tmp_to_exp: rhs is call %s\n" vi.vname);
            None)
  else (
    if !debug then
      ignore (E.log "tmp_to_exp: %s didn't match form or nofrm\n" vi.vname);
    None)
...
let ae_lv_fwd_subst ?(propStrings : bool = false) data sid e fd nofrm =
  ...
  let e' =
    visitCilExpr
      (lvalXformClass (ae_lval_to_exp ~propStrings) data sid fd nofrm)
      e
  in
  (e', !ae_lval_to_exp_change)

let ae_simp_fwd_subst data e nofrm = ae_fwd_subst data (-1) e dummyFunDec nofrm
...
let tmp_to_const iosh sid vi fd nofrm =
  if nofrm || check_forms vi.vname forms then
    ...
    | None -> None
    | Some ios -> (
        let defido = try RD.IOS.choose ios with Not_found -> None in
        match defido with
        | None -> None
        | Some defid -> (
                                ...
                                | Some (RD.RDExp (Const c'), _, defiosh) ->
                                    if Util.equals c c' then
                                      ...
                                      | None ->
                                          E.s
                                            (E.error
                                               "tmp_to_const: defid has no \
                                                statement")
                                      | Some stm ->
                                          ok_to_replace vi iosh sid defiosh
                                            stm.sid fd (RD.RDExp (Const c'))
                                    else false
                                | _ -> false))
                          ios
                      in
                      if same then (
                        tmp_to_const_change := true;
                        Some (Const c))
                      else None
                    else None)
            | _ -> None))
...
class expTempElimClass (fd : fundec) =
  object (self)
    inherit RD.rdVisitorClass
    ...
    method vexpr e =
      let do_change iosh vi =
        ...
        | Some id -> (
            ...
            | Some ((RD.RDExp e as r), dsid, defiosh) ->
                if !debug then
                  ignore (E.log "Can I replace %s with %a?\n" vi.vname d_exp e);
                if ok_to_replace vi iosh sid defiosh dsid fd r then (
      ...
      | Lval (Var vi, NoOffset) ->
          if check_forms vi.vname forms then
            ...
            | None -> (
                ...
                | None ->
                    if !debug then
                      ignore (E.log "%s in statement w/o RD info\n" vi.vname);
                    DoChildren)
          else DoChildren
      | _ -> DoChildren
  end

class expLvTmpElimClass (fd : fundec) =
  object (self)
...
class incdecTempElimClass (fd : fundec) =
  object (self)
    inherit RD.rdVisitorClass
        ...
        | Some id -> (
            ...
            | Some ((RD.RDExp e as r), _, defiosh) -> (
                ...
                | Some (curdef_stmt_id, redefid, rhsvi, b) ->
                    if !debug then
                      ignore
                        (E.log "No, but I can replace it with a post-inc/dec\n");
                    if !debug then
                      ignore
                        (E.log "cdsi: %d redefid: %d name: %s\n" curdef_stmt_id
                           redefid rhsvi.vname);
...
class callTempElimClass (fd : fundec) =
  object (self)
    inherit RD.rdVisitorClass
    ...
    method vexpr e =
      ...
      | Lval (Var vi, NoOffset) ->
          if check_forms vi.vname forms then
            ...
            if IH.mem iioh vi.vid then (
              IH.replace iioh vi.vid None;
              DoChildren)
            else
              match cur_rd_dat with
              | Some (_, s, iosh) -> do_change iosh vi
              | None -> (
                  ...
                  | Some (_, s, iosh) ->
                      if !debug then
                        ignore
                          (E.log "Try to change %s:%d outside of instruction.\n"
                             vi.vname vi.vid);
                      do_change iosh vi
                  | None ->
                      if !debug then
...
let rm_unused_locals fd =
  ...
  let used =
    List.fold_left
      (fun u s ->
        let u', d' = UD.computeDeepUseDefStmtKind s.skind in
        UD.VS.union u (UD.VS.union u' d'))
      UD.VS.empty fd.sbody.bstmts
  in
...
let is_volatile vi =
  let vi_vol =
    List.exists (function Attr ("volatile", _) -> true | _ -> false) vi.vattr
  in
  let typ_vol =
    List.exists
      (function Attr ("volatile", _) -> true | _ -> false)
      (typeAttrs vi.vtype)
  in
...
(* This is different from dead code elimination because
   ...
   are also considered *)
class unusedRemoverClass : cilVisitor =
  object (self)
    inherit nopCilVisitor
    ...
    method vfunc f =
      ...
      let unused =
        List.fold_left
          (fun un vi ->
            if UD.VS.mem vi used then un
            else (
              if !debug then
                ignore (E.log "unusedRemoverClass: %s is unused\n" vi.vname);
              UD.VS.add vi un))
          UD.VS.empty f.slocals
      in
      ...
      let good_var vi =
        is_volatile vi
        || (* have to keep if it's volatile *)
        (not (UD.VS.mem vi unused))
        (* have to keep if it's used and if *)
        && ((not (IH.mem iioh vi.vid))
           ||
           (* it's not getting eliminated during pp *)
    ...
    method vstmt stm =
      ...
      let check_incdec vi e =
        if IH.mem idDefHash vi.vid then (
          ...
          match findf_in_pl stm.sid pl with
          | (sid, redefid) :: l -> (
              ...
              | None ->
                  if !debug then
                    ignore
                      (E.log "check_incdec: couldn't find rhs for def %d\n"
                         redefid);
                  false
              | Some (rhs, _, indiosh) -> (
                  match rhs with
                  | RD.RDCall _ ->
                      if !debug then
                        ignore (E.log "check_incdec: rhs not an expression\n");
                      false
                  | RD.RDExp e' ->
                      if Util.equals e e' then true
                        ...
                        if !debug then
                          ignore
                            (E.log
                               "check_incdec: rhs of %d: %a, and needed redef \
                                %a not equal\n"
          ...
          | [] ->
              if !debug then
                ignore
                  (E.log
                     "check_incdec: current statement not in list: %d. %s = %a\n"
                     stm.sid vi.vname d_exp e);
              false)
        else (
          if !debug then
            ignore (E.log "check_incdec: %s not in idDefHash\n" vi.vname);
          false)
      in
      ...
      let will_be_call e =
        ...
        | Lval (Var vi, NoOffset) -> (
            ...
            else match IH.find iioh vi.vid with None -> false | Some _ -> true)
        | _ -> false
      in
      ...
      let good_instr i =
        ...
        | Call (Some (Var vi, _), _, _, _) -> (
            (* If not in the table or entry is None,
               then it's good *)
            (not (IH.mem iioh vi.vid))
            || match IH.find iioh vi.vid with None -> true | Some _ -> false)
        | Asm (_, _, slvlst, _, _, _) ->
            ...
            List.iter
              (fun (_, s, lv) ->
                match lv with
                | Var vi, _ ->
                    if List.mem vi cur_func.slocals then ()
      ...
      let call_fixer i =
        ...
        | Call (Some (Var vi, _), e, el, l) as c ->
            if UD.VS.mem vi unused_set then Call (None, e, el, l) else c
        | _ -> i
      in
  ...
  b.bstmts <-
    List.fold_right
      (fun s acc ->
        ...
        | Block ib ->
            fold_blocks ib;
            if List.length ib.battrs = 0 && List.length s.labels = 0 then
              ib.bstmts @ acc
            else s :: acc
        | Instr il when il = [] && s.labels = [] -> acc
        | _ -> s :: acc)
      b.bstmts []
...
let eliminate_temps f =
  ...
  RD.clearMemos ();
  (* we changed instructions and invalidated the "cache" *)

...
let considerVariableUse : (varinfo -> bool) ref = ref (fun _ -> true)

(** Say if you want to consider a variable def *)
let considerVariableDef : (varinfo -> bool) ref = ref (fun _ -> true)

(** Say if you want to consider a variable addrof as a use *)
let considerVariableAddrOfAsUse : (varinfo -> bool) ref = ref (fun _ -> true)

(** Say if you want to consider a variable addrof as a def *)
let considerVariableAddrOfAsDef : (varinfo -> bool) ref = ref (fun _ -> false)
...
let extraUsesOfExpr : (exp -> VS.t) ref = ref (fun _ -> VS.empty)
...
class useDefVisitorClass : cilVisitor =
  object (self)
    inherit nopCilVisitor

    method! vvrbl (v : varinfo) =
      if !considerVariableDef v && not !onlyNoOffsetsAreDefs then
        varDefs := VS.add v !varDefs;
      if !considerVariableDef v && !onlyNoOffsetsAreDefs then
   ...
   * all uses of variables in expressions ! *)
    ...
    method! vlval (l : lval) =
      if !onlyNoOffsetsAreDefs then
        match l with
        | Var vi, NoOffset ->
            if !considerVariableDef vi then varDefs := VS.add vi !varDefs;
            SkipChildren
   ...
   *  DoChildren *)
    ...
    method! vexpr (e : exp) =
      ...
      if not (VS.is_empty extra) then varUsed := VS.union extra !varUsed;
      match e with
      | Lval (Var v, off) ->
          ignore (visitCilOffset (self :> cilVisitor) off);
          if !considerVariableUse v then varUsed := VS.add v !varUsed;
          SkipChildren (* So that we do not see the v *)
      | AddrOf (Var v, off) | StartOf (Var v, off) ->
          ignore (visitCilOffset (self :> cilVisitor) off);
          if !considerVariableAddrOfAsUse v then varUsed := VS.add v !varUsed;
          if !considerVariableAddrOfAsDef v then varDefs := VS.add v !varDefs;
          SkipChildren
      | (SizeOfE _ | AlignOfE _) when !ignoreSizeof -> SkipChildren
      | _ -> DoChildren
    ...
    method! vinst i =
      ...
      match i with
      | Call
          (None, (Lval (Var vi, NoOffset) as f), [ valist; SizeOf t; adest ], _)
      ...
      | Call (_, Lval (Var vi, _), _, _) when vi.vname = "__builtin_va_arg" ->
          E.s (bug "bad call to %s" vi.vname)
      | Call (lvo, f, args, _) -> doCall f lvo args
      | Asm (_, _, slvl, _, _, _) ->
          List.iter
            (fun (_, s, lv) ->
              match lv with
              | Var v, off -> if s.[0] = '+' then varUsed := VS.add v !varUsed
              | _ -> ())
            slvl;
          DoChildren
      | _ -> DoChildren
...
let computeUseDefInstr ?(acc_used = VS.empty) ?(acc_defs = VS.empty) (i : instr)
    : VS.t * VS.t =
  varUsed := acc_used;
  varDefs := acc_defs;
 ...
 * the nested blocks. *)
let computeUseDefStmtKind ?(acc_used = VS.empty) ?(acc_defs = VS.empty)
  ...
  let _ =
    ...
    | Loop (_, _, _, _) -> ()
    | Switch (e, _, _, _) -> ve e
    | Instr il -> List.iter (fun i -> ignore (visitCilInstr useDefVisitor i)) il
...
let rec computeDeepUseDefStmtKind ?(acc_used = VS.empty) ?(acc_defs = VS.empty)
    (sk : stmtkind) : VS.t * VS.t =
  let handle_block b =
    List.fold_left
      (fun (u, d) s ->
...
let computeUseLocalTypes ?(acc_used = VS.empty) (fd : fundec) =
  List.fold_left
    (fun u vi ->

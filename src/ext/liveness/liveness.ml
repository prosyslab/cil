...
let debug_print () vs =
  VS.fold
    (fun vi d ->
      d ++ text "name: " ++ text vi.vname ++ text " id: " ++ num vi.vid
      ++ text " ")
    vs nil
  ++ line
...
let min_print () vs =
  VS.fold
    (fun vi d ->
      d ++ text vi.vname ++ text "(" ++ d_type () vi.vtype ++ text ")"
      ++ text ",")
    vs nil
  ++ line
...
module LiveFlow = struct
  ...
  let combineStmtStartData (stm : stmt) ~(old : t) (now : t) =
    if not (VS.compare old now = 0) then Some (VS.union old now) else None
  ...
  let doStmt stmt =
    ...
    let handle_stm vs =
      ...
      | Instr _ -> vs
      | s ->
          let u, d = UD.computeUseDefStmtKind s in
...
let computeLiveness fdec =
  ...
  with E.Error ->
    ignore
      (E.log "Liveness failed on function:\n %a\n" d_global
         (GFun (fdec, locUnknown)));
    E.s "Bug in Liveness compute"

let getLiveSet sid =
  try Some (IH.find LiveFlow.stmtStartData sid) with Not_found -> None
...
let instrLiveness (il : instr list) (stm : stmt) (vs : VS.t) (out : bool) :
    VS.t list =
  let proc_one vsl i =
    match vsl with
    | [] ->
        if !ignore_inst i then vs :: vsl
        else
          let u, d = UD.computeUseDefInstr i in
...
class livenessVisitorClass (out : bool) =
  object (self)
    inherit nopCilVisitor
    val mutable sid = -1
            ...
            List.fold_left
              (fun (dead, live) stm ->
                let dvs =
                  ...
                  match stm.skind with
                  | Instr _ | Block _ -> VS.diff (getPostLiveness stm) vs
                  | _ ->
                      VS.diff
                        (VS.union (getLiveness stm) (getPostLiveness stm))
                        vs
                in
                (VS.union dead dvs, VS.union live (getPostLiveness stm)))
              (VS.empty, VS.empty) stm.preds
          in
          if !debug then
            E.log "deadVis: before %a, %a die, %a come to live\n" d_stmt stm
              debug_print dead debug_print live;
          post_dead_vars <- dead;
      ...
      try
        ...
        if !debug then
          E.log "deadVis: at %a, liveout: %a, inlive: %a, post_dead_vars: %a\n"
            d_instr i debug_print data debug_print inlive debug_print
            post_dead_vars;
        DoChildren
      with Failure _ ->
...
let print_everything () =
  let d =
    IH.fold
      (fun i vs d -> d ++ num i ++ text ": " ++ LiveFlow.pretty () vs)
...
class doFeatureClass =
  object (self)
    ...
    method! vstmt s =
      if List.exists match_label s.labels then (
        try
...
let do_live_feature (f : file) = visitCilFile (new doFeatureClass) f

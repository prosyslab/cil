...
let big_int_of_cilint (c : cilint) : big_int =
  match c with Small i -> big_int_of_int i | Big b -> b

let cilint_of_big_int (b : big_int) : cilint =
  if is_int_big_int b then Small (int_of_big_int b) else Big b
...
let b op c1 c2 =
  cilint_of_big_int (op (big_int_of_cilint c1) (big_int_of_cilint c2))
...
let is_zero_cilint (c : cilint) : bool =
  match c with Small i -> i = 0 | Big b -> sign_big_int b = 0
...
let rec cilint_of_int64 (i64 : int64) : cilint =
  ...
  then Small (Int64.to_int i64)
    ...
    in
    Big (loop i64 unit_big_int zero_big_int)
...
let string_of_cilint (c : cilint) : string =
  match c with Small i -> string_of_int i | Big b -> string_of_big_int b
...
let div0_cilint (c1 : cilint) (c2 : cilint) =
  ...
  | _ ->
      ...
      if lt_big_int b1 zero_big_int && not (eq_big_int r zero_big_int) then
        if gt_big_int b2 zero_big_int then Big (succ_big_int q)
        else Big (pred_big_int q)
      else Big q
...
let logop op c1 c2 =
  ...
  | _ ->
      ...
      in
      cilint_of_big_int (loop b1 b2 unit_big_int zero_big_int)
...
let shift_left_cilint (c1 : cilint) (n : int) : cilint =
  cilint_of_big_int
    (mult_big_int (big_int_of_cilint c1) (power_int_positive_int 2 n))
...
let truncate_signed_cilint (c : cilint) (n : int) : cilint * truncation =
  ...
  | Small i when n < Nativeint.size - 2 ->
      ...
      let tval =
        ...
        if bits < max then bits
        else (* and fill with 1 bits on the left if it is *)
          bits - truncmax
      in
      let trunc =
        if i >= max || i < -max then
          if i >= truncmax then BitTruncation else ValueTruncation
        else NoTruncation
      in
      (Small tval, trunc)
  | _ ->
      ...
      let tval =
        if lt_big_int bits max then bits else sub_big_int bits truncmax
      in
      let trunc =
        if ge_big_int b max || lt_big_int b (minus_big_int max) then
          if ge_big_int b truncmax then BitTruncation else ValueTruncation
        else NoTruncation
      in
  ...
  | Small i when n < Nativeint.size - 2 ->
      ...
      let trunc =
        if i >= truncmax || i < 0 then
          if i < -max then BitTruncation else ValueTruncation
        else NoTruncation
      in
      (Small bits, trunc)
  | _ ->
      ...
      let trunc =
        if ge_big_int b truncmax || lt_big_int b zero_big_int then
          if lt_big_int b (minus_big_int max) then BitTruncation
...
let is_int_cilint (c : cilint) : bool =
  match c with Small _ -> true | Big b -> is_int_big_int b

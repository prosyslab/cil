...
functor
  ...
  struct
    ...
    let reachedStatement (s : stmt) (d : T.t) : unit =
      ...
      if loc != locUnknown then currentLoc := get_stmtLoc s.skind;
      (* see if we know about it already *)
      E.pushContext (fun _ ->
          dprintf "Reached statement %d with %a" s.sid T.pretty d);
      let newdata : T.t option =
        try
          let old = IH.find T.stmtStartData s.sid in
          match T.combinePredecessors s ~old d with
          | None ->
              (* We are done here *)
              if !T.debug then
                ignore
          ...
          | Some d' ->
              ...
              if !T.debug then
                ignore
                  (E.log "FF(%s): weaken data for block %d: %a\n" T.name s.sid
                     T.pretty d');
              Some d'
        with Not_found ->
          (* was bottom before *)
          ...
          if !T.debug then
            ignore
              (E.log "FF(%s): set data for block %d: %a\n" T.name s.sid T.pretty
                 d');
          Some d'
      in
      E.popContext ();
      ...
      | Some d' ->
          ...
          if
            ...
            && not
                 (Queue.fold
                    (fun exists s' -> exists || s'.sid = s.sid)
                    false worklist)
          then Queue.add s worklist

    (** Get the two successors of an If statement *)
    let ifSuccs (s : stmt) : stmt * stmt =
      let fstStmt blk =
        match blk.bstmts with [] -> Cil.dummyStmt | fst :: _ -> fst
      in
      match s.skind with
      | If (e, b1, b2, _) ->
          ...
          let oneFallthrough () =
            let fallthrough =
              List.filter (fun s' -> thenSucc != s' && elseSucc != s') s.succs
            in
          ...
          let stmtOrFallthrough s' =
            if s' == Cil.dummyStmt then oneFallthrough () else s'
          in
          (stmtOrFallthrough thenSucc, stmtOrFallthrough elseSucc)
    ...
    let processStmt (s : stmt) : unit =
      ...
      | (SDefault | SUse _) as act ->
          ...
          let after : T.t =
            ...
            | TryExcept _ | TryFinally _ | Switch _ | Loop _ | Return _
            | Block _ ->
                curr
          in
          currentLoc := get_stmtLoc s.skind;
                ...
                if thenGuard = GDefault && elseGuard = GDefault then
                  ...
                  let doBranch succ guard =
                    ...
                    | GUnreachable ->
                        if !T.debug then
                          ignore
                            (E.log "FF(%s): Not exploring branch to %d\n" T.name
                               succ.sid);
      ...
      List.iter
        (fun s ->
          if not (IH.mem T.stmtStartData s.sid) then
            E.s
              (E.error "FF(%s): initial stmt %d does not have data" T.name s.sid))
        sources;
      if !T.debug then ignore (E.log "\nFF(%s): processing\n" T.name);
      let rec fixedpoint () =
        if !T.debug && not (Queue.is_empty worklist) then
          ignore
            (E.log "FF(%s): worklist= %a\n" T.name
               (docList (fun s -> num s.sid))
               (List.rev (Queue.fold (fun acc s -> s :: acc) [] worklist)));
        let keepgoing =
          try
            ...
            true
          with Queue.Empty ->
            if !T.debug then ignore (E.log "FF(%s): done\n\n" T.name);
            false
        in
        if keepgoing then fixedpoint ()
      in
      fixedpoint ()
  end
...
functor
  ...
  struct
    let getStmtStartData (s : stmt) : T.t =
      ...
      with Not_found ->
        E.s
          (E.bug "BF(%s): stmtStartData is not initialized for %d: %a" T.name
             s.sid d_stmt s)
    ...
    let processStmt (s : stmt) : bool =
      if !T.debug then ignore (E.log "FF(%s).stmt %d\n" T.name s.sid);
      ...
      let d : T.t =
        ...
        | (Default | Post _) as action -> (
            ...
            let res' =
              ...
              | _ -> res
            in
            match action with Post f -> f res' | _ -> res')
      in
      ...
      | Some d' ->
          ...
          if !T.debug then
            ignore
              (E.log "BF(%s): set data for block %d: %a\n" T.name s.sid T.pretty
                 d');
          IH.replace T.stmtStartData s.sid d';
          true
                  ...
                  if
                    (not
                       (Queue.fold
                          ...
                          false worklist))
                    && T.filterStmt p s
                  then Queue.add p worklist)
                s.preds;
            true
          with Queue.Empty ->
            if !T.debug then ignore (E.log "BF(%s): done\n\n" T.name);
            false
        in
        if keepgoing then fixedpoint ()
      in

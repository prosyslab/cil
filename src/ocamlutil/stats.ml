...
let top = { name = "TOTAL"; time = 0.0; ncalls = 0; sub = [] }
...
let print chn msg =
  ...
  let rec prTree ind node =
    Printf.fprintf chn "%s%-25s      %6.3f s" (String.make ind ' ') node.name
      node.time;
    if node.ncalls <= 0 then output_string chn "\n"
    else if node.ncalls = 1 then output_string chn "  (1 call)\n"
    else Printf.fprintf chn "  (%d calls)\n" node.ncalls;
  ...
  Printf.fprintf chn
    ...
    (printM (gc.Gc.minor_words +. gc.Gc.major_words -. gc.Gc.promoted_words))
    (printM (float_of_int gc.Gc.top_heap_words))
    (printM gc.Gc.minor_words) (printM gc.Gc.major_words)
    (printM gc.Gc.promoted_words)
    gc.Gc.minor_collections gc.Gc.major_collections gc.Gc.compactions;

  ()

(* Get the current time, in seconds *)
let get_current_time () : float = (Unix.times ()).Unix.tms_utime
...
let repeattime limit str f arg =
  ...
  let rec repeatf count =
    ...
    in
    let diff = get_current_time () -. start in
    if diff < limit then repeatf (count + 1)
    else (
      finish diff;
...
let time str f arg =
  if !timerMode = Disabled then f arg else repeattime 0.0 str f arg

...
let split_len (len : int) : int * int =
  if len <= Sys.max_array_length then (len, 0)
  else (Sys.max_array_length, len - Sys.max_array_length)

let split_idx (idx : int) : int option =
  if idx < Sys.max_array_length then None else Some (idx - Sys.max_array_length)
...
let rec blit (src : 'a t) (srcidx : int) (dst : 'a t) (dstidx : int) (len : int)
    : unit =
  ...
  try
    ...
    if len2 > 0 then blit (List.tl src) 0 (List.tl dst) 0 len2
  with Failure _ -> raise (Invalid_argument "Longarray.blit")
...
let rec fill (a : 'a t) (idx : int) (len : int) (elt : 'a) : unit =
  try
    ...
    | None ->
        ...
        if end2 > 0 then fill (List.tl a) 0 end2 elt
    | Some idx' -> fill (List.tl a) idx' len elt
  with Failure _ -> raise (Invalid_argument "Longarray.fill")

let rec length (a : 'a t) : int =
  match a with hd :: tl -> Array.length hd + length tl | [] -> 0
...
let rec get (a : 'a t) (i : int) : 'a =
  ...
  with Failure _ -> raise (Invalid_argument "(get) index out of bounds")
...
let rec set (a : 'a t) (i : int) (elt : 'a) : unit =
  ...
  with Failure _ -> raise (Invalid_argument "(set) index out of bounds")

let rec copy (a : 'a t) : 'a t =
  match a with hd :: tl -> Array.copy hd :: copy tl | [] -> []

let rec map (fn : 'a -> 'b) (a : 'a t) : 'b t =
  match a with hd :: tl -> Array.map fn hd :: map fn tl | [] -> []

let docArray ?(sep = chr ',') (doit : int -> 'a -> doc) () (elements : 'a t) =
  ...
  if len = 0 then nil
    ...
    let rec loop (acc : doc) i =
      if i >= len then acc
        ...
        let fi = doit i (get elements i) in
